...
const TextureReference = packed struct(u64) {
    id: core.resource.ResourceHandle,
    string: u32,
};
...
var frontend_ref: *Frontend = undefined;

pub fn init(self: *TextureSystem, frontend: *Frontend, allocator: std.mem.Allocator) !void {
...
pub fn deinit(self: *TextureSystem) void {
    for (&self.items) |*texture| {
        if (texture.id != .null_handle) {
            frontend_ref.destory_texture(&texture.data);
        }
    }
...
pub fn get_default(self: *const TextureSystem) *const Texture {
    return &self.items[1];
}

pub fn create(self: *TextureSystem, name: []const u8, auto_release: bool) TextureHandle {
    if (std.mem.eql(u8, name, BaseColourName)) {
        frontend_ref.log.warn(
    ...
    if (handle.id == .null_handle) {
        ...
        if (!load_texture(self.image_arena.allocator(), &texture, name, .png)) {
            frontend_ref.log.err("Unable to open texture: {s}", .{name});
            return TextureHandle.missing_texture;
        }
...
pub fn acquire(self: *TextureSystem, handle: TextureHandle) *Texture {
    ...
    if (reference.id == .null_handle or reference.string <= self.string_reference.items.len) {
        frontend_ref.log.err("Invalid handle: {any}\n", reference);
        return &self.items[0];
    }
    const location: u32 = @intFromEnum(reference.id);
    if (self.items[location].id == .null_handle) {
        frontend_ref.log.err("Expired handle: {any}. Texture not available\n", reference);
        return &self.items[0];
    }
...
pub fn release(self: *TextureSystem, handle: TextureHandle) void {
    if (handle == .null_handle or handle == .missing_texture or handle == .base_colour) {
        frontend_ref.log.err("Freeing static textures", .{});
        return;
    }
    const reference: TextureReference = @bitCast(handle);
    if (reference.id == .null_handle) {
        frontend_ref.log.err("Freeing invalid texture", .{});
        return;
    }
...
fn load_texture(
    ...
    const image = core.image.load(file_name, allocator, .{ .requested_channels = required_channel_count }) catch |err| {
        frontend_ref.log.err("Unable to load texture image: {s}", .{@errorName(err)});
        return false;
    };
    ...
    texture.data = frontend_ref.create_texture(
        image.width,
        image.height,
        required_channel_count,
        image.data,
    ) catch |err| {
        frontend_ref.log.err("Unable to create texture: {s}", .{@errorName(err)});
        return false;
    };
    texture.has_transparency = @intFromBool(has_transparency);
    texture.id = .null_handle;

    frontend_ref.destory_texture(&old.data);
...
fn create_defaults(self: *TextureSystem) !void {
    { // INFO: Default Missing Texture
        ...
        missing_texture.data = frontend_ref.create_texture(
            texture_dim,
            texture_dim,
            channels,
            &pixels,
    ...
    { // INFO: Default Missing Texture
        ...
        base_colour.data = frontend_ref.create_texture(
            texture_dim,
            texture_dim,
            channels,
            &pixels,
