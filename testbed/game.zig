...
pub fn updateAndRender(
    ...
    if (engine.input.isMouseButtonDown(.left)) {
        std.log.info(
            "Mouse button pressed at {d}, {d}",
            .{ engine.input.mouse_position_current.x, engine.input.mouse_position_current.y },
        );
    }
    ...
    if (engine.input.keyReleasedThisFrame(.a)) {
